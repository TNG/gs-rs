...
fn update_H_submatrix(H: &mut DMatrix<f64>, added_matrix: &Matrix<f64, U3, U3, ArrayStorage<f64,U3,U3>>, var: &Box<dyn Variable>) {
...
fn update_b_subvector(b: &mut DVector<f64>, added_vector: &Vector<f64, U3, ArrayStorage<f64,U3,U1>>, var: &Box<dyn Variable>) {
    let range = var.get_range().unwrap();
