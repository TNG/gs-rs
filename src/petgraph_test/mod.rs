...
use crate::petgraph_test::FactorTest::{OdoFactor, PositionFactor};
use nalgebra::{MatrixN, Dynamic, VecStorage, DMatrix};
...
impl From<FactorGraphModel> for FactorGraph<'_> {
    fn from(model: FactorGraphModel) -> Self {
        let mut csr: Csr<Box<dyn Variable>, FactorTest, Undirected, usize> = Csr::new();
